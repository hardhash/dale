...
2. 图模型-图神经网络更新：
    ...
    - GCN
      - GCN基本思想
    - PyG(Pytorch-Geometric)框架做如下删改：
      - 安装根据最新文档（2023年11月）进行修改（已经不再支持python3.8以下版本）
      - 删除原图模型通用框架撰写
      - 删除原PyG数据格式
    - PyG(Pytorch-Geometric)框架新增：
      - PyG基本使用
      - 基于点的图预测任务
      - 构建自己的数据集，包括数据构建通用范式和雅虎电商数据实战

3. 竞赛总结板块
    
    - 新增【(2023 OCR)厦门国际银行建模大赛(**TOP 6%**)】
    - 新增【(2024 CV)Kaggle: HMS - Harmful Brain Activity Classification】

4. 论文精读板块

    - 新增可解释机器学习模块
      - LIME，一种通用的机器学习/深度学习局部到全局的可解释性范式
      - CAM串讲，类激活热力图，用于视觉模型可解释性
